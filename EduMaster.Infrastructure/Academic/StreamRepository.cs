...
WHERE LevelId = @LevelId AND Name = @Name AND (@ExcludeId IS NULL OR Id <> @ExcludeId);",
    ...
    {
        var connection = await _session.GetOpenConnectionAsync(cancellationToken);
        // D-55 (مفعَّل منذ 2.4): أفواج فعّالة مقيّدة بهذه الشعبة تمنع تعطيلها
        var count = await connection.ExecuteScalarAsync<int>(
        new CommandDefinition(
        @"SELECT COUNT(*) FROM ClassGroupStreams cgs
JOIN ClassGroups cg ON cg.Id = cgs.ClassGroupId
WHERE cgs.StreamId = @StreamId AND cg.IsActive = 1;",
        new { StreamId = id },
        transaction: _session.CurrentTransaction,
        cancellationToken: cancellationToken));
        return count > 0;
