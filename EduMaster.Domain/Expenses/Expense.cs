...
public sealed class Expense
{
    ...
    private Expense(int academicYearId, int expenseCategoryId, int treasuryAccountId, DateOnly expenseDate, long amountCentimes, string? note,
    ...
    {
        Validate(academicYearId, expenseCategoryId, treasuryAccountId, amountCentimes, note);
    ...
    public static Expense Create(int academicYearId, int expenseCategoryId, int treasuryAccountId, DateOnly expenseDate,
        long amountCentimes, string? note, DateTime createdAtUtc, int? createdByUserId)
        => new(academicYearId, expenseCategoryId, treasuryAccountId, expenseDate, amountCentimes, note, false,
            createdAtUtc, createdByUserId, null, null, null, null);

    public static Expense Load(int id, int academicYearId, int expenseCategoryId, int treasuryAccountId, DateOnly expenseDate,
    ...
    {
        var expense = new Expense(academicYearId, expenseCategoryId, treasuryAccountId, expenseDate, amountCentimes, note, isDeleted,
    ...
    public void Update(int academicYearId, int expenseCategoryId, int treasuryAccountId, DateOnly expenseDate, long amountCentimes,
    ...
    private static void Validate(int academicYearId, int expenseCategoryId, int treasuryAccountId, long amountCentimes, string? note)
    {
        if (academicYearId <= 0)
        ...
        if (treasuryAccountId <= 0)
            throw new DomainException("المصروف يجب أن يرتبط بحساب مالي.");
        if (amountCentimes <= 0)
