...
/// <summary>محرر الموظف (شخص + ملف) — مرآة محرر الأستاذ بقالب D-41 · الوظيفة إلزامية (D-115)</summary>
public sealed class EmployeeEditorViewModel : BaseViewModel, IDialogViewModel
{
    ...
    public string Title => _editingEmployeeId is null ? "موظف جديد" : "تعديل الموظف";
    ...
    public void InitializeForEdit(EmployeeListItem item)
    {
        _editingEmployeeId = item.Id;
    ...
    private async Task SaveAsync()
    {
        ...
        {
            ErrorMessage = "أدخل وظيفة الموظف.";
            return;
        }
        ...
        try
        {
            ...
            if (_editingEmployeeId is null)
            {
                ...
                var result = await handler.ExecuteAsync(new CreateEmployeeRequest(
                    ...
                    Phone, Phone2, Email, Address, JobTitle, Notes,
                    PhotoSourcePath: _photoRemoved ? null : _photoSourcePath));

                if (!HandleSaveResult(result.IsSuccess, result.ErrorMessage, result.ErrorType, "أُضيف الموظف بنجاح ✔"))
                    return;
            }
            else
            {
                var handler = scope.ServiceProvider.GetRequiredService<UpdateEmployeeHandler>();
                var result = await handler.ExecuteAsync(new UpdateEmployeeRequest(
                    _editingEmployeeId.Value, FirstName, LastName, FatherName, birthDate, SelectedGender?.Value,
                    Phone, Phone2, Email, Address, JobTitle, Notes));

                if (!HandleSaveResult(result.IsSuccess, result.ErrorMessage, result.ErrorType, "حُفظت بيانات الموظف ✔"))
                    return;
