...
public sealed class PeopleViewModel : BaseViewModel
{
    ...
    IDialogService dialogs)
    {
        ...
        AssignEmployeeRoleCommand = new AsyncRelayCommand(AssignEmployeeRoleAsync, () => SelectedPerson is { IsActive: true });
    }
    ...
    public AsyncRelayCommand AssignEmployeeRoleCommand { get; }
    ...
    private void RaiseActionCommandsCanExecute()
    {
        ...
        AssignEmployeeRoleCommand.RaiseCanExecuteChanged();
    }
    ...
    // F5 — دفعة B-1 (ب-2 المحسوم): «أضف كموظف» فوق شخص قائم — مرآة الدورين أعلاه
    private async Task AssignEmployeeRoleAsync()
    {
        var person = SelectedPerson;
        if (person is null) return;
        var dialog = _services.GetRequiredService<AssignEmployeeRoleViewModel>();
        dialog.Initialize(person);
        await _dialogs.ShowDialogAsync(dialog, "إضافة ملف موظف");
    }
}